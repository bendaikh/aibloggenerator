<template>
    <OrganizationLayout>
        <Head title="Websites" />

        <div class="p-8">
            <div class="flex justify-between items-center mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-white">Websites</h1>
                    <p class="text-gray-400 mt-1">Manage all your websites</p>
                </div>
                <Link
                    :href="route('organization.websites.create')"
                    class="inline-flex items-center px-6 py-3 bg-emerald-500 text-white font-semibold rounded-lg hover:bg-emerald-600 transition"
                >
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    Create New Website
                </Link>
            </div>

            <div v-if="websites.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div
                    v-for="website in websites"
                    :key="website.id"
                    class="bg-[#1a1a1a] rounded-2xl border border-[#2a2a2a] overflow-hidden hover:border-emerald-500/50 transition-all"
                >
                    <div class="h-32 bg-gradient-to-br from-amber-500/20 to-yellow-500/20 flex items-center justify-center">
                        <div class="w-16 h-16 bg-gradient-to-br from-amber-500 to-yellow-500 rounded-xl flex items-center justify-center">
                            <span class="text-2xl font-bold text-white">{{ website.name.charAt(0) }}</span>
                        </div>
                    </div>
                    <div class="p-6">
                        <h2 class="text-xl font-bold text-white mb-2">{{ website.name }}</h2>
                        <p v-if="website.description" class="text-gray-400 text-sm mb-4 line-clamp-2">
                            {{ website.description }}
                        </p>
                        <div class="flex items-center gap-4 mb-4 text-sm text-gray-400">
                            <span class="flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                                {{ website.articles_count }} articles
                            </span>
                            <span class="flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                                </svg>
                                {{ website.categories_count }} categories
                            </span>
                        </div>
                        <div class="flex items-center justify-between mb-4">
                            <span :class="[
                                'px-3 py-1 rounded-full text-xs font-semibold',
                                website.is_active ? 'bg-emerald-900/50 text-emerald-400' : 'bg-red-900/50 text-red-400'
                            ]">
                                {{ website.is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </div>
                        <div class="flex gap-2">
                            <Link
                                :href="route('superadmin.dashboard', { website: website.id })"
                                class="flex-1 text-center px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition font-medium"
                            >
                                Manage
                            </Link>
                            <Link
                                :href="route('organization.websites.edit', website.id)"
                                class="px-4 py-2 bg-[#252525] text-white rounded-lg hover:bg-[#303030] transition"
                            >
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </Link>
                            <a
                                :href="website.url"
                                target="_blank"
                                class="px-4 py-2 bg-[#252525] text-white rounded-lg hover:bg-[#303030] transition"
                                title="View Live Site"
                            >
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                                </svg>
                            </a>
                            <button
                                @click="deleteWebsite(website)"
                                class="px-4 py-2 bg-red-600/20 text-red-400 rounded-lg hover:bg-red-600/30 transition"
                                title="Delete Website"
                            >
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                            </button>
                        </div>
                    </div>
                </div>
            </div>

            <div v-else class="text-center py-16 bg-[#1a1a1a] rounded-2xl border border-[#2a2a2a]">
                <div class="w-20 h-20 bg-[#252525] rounded-full flex items-center justify-center mx-auto mb-6">
                    <svg class="w-10 h-10 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                    </svg>
                </div>
                <h3 class="text-2xl font-bold text-white mb-2">No websites yet</h3>
                <p class="text-gray-400 mb-8">Create your first website to get started!</p>
                <Link
                    :href="route('organization.websites.create')"
                    class="inline-flex items-center px-8 py-4 bg-emerald-500 text-white font-semibold rounded-xl hover:bg-emerald-600 transition"
                >
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    Create Website
                </Link>
            </div>
        </div>
    </OrganizationLayout>
</template>

<script setup>
import { Head, Link, router } from '@inertiajs/vue3';
import OrganizationLayout from '@/Layouts/OrganizationLayout.vue';

defineProps({
    websites: Array
});

const deleteWebsite = (website) => {
    if (confirm(`Are you sure you want to delete "${website.name}"? This action cannot be undone.`)) {
        router.delete(route('organization.websites.destroy', website.id), {
            preserveScroll: true
        });
    }
};
</script>

